...
import sys
sys.path.append('/data/mly/CDA/pretrain')
from autoencoder import DNET_lmmd, DNET_mmd
sys.path.append('/data/mly/CDA/lmmd') 
# sys.path.append('/data/mly/CDA/lmmd') 
from models import M2SFE_lmmd
...
def load_data(root_path, src, tar, batch_size):
    ...
    X_train, Y_train, X_test, Y_test, X_SNR_train, X_SNR_test = load_modulation_SNR_H(data='04c')#load_modulation_lt ,load_modulation_SNR_H_lt
    NumTrainData = len(X_train)
...
if __name__ == '__main__':
    ...
    model = DNET_mmd()
    # model.load_state_dict(torch.load('/data/mly/CDA/pretrain/AE.pth'))
    model.cuda()
    dataloaders = load_data(args.root_path, args.src,
                            args.tar, args.batch_size)
    model.eval()
    acc_src = test_with_snr(model, dataloaders[-2])
    print('acc for src:{:.4f}'.format(acc_src))
    acc_tgt = test_with_snr(model, dataloaders[-1])
    print('acc for tgt:{:.4f}'.format(acc_tgt))
    max_acc = 0
    stop = 0
    ...
    for epoch in range(1, args.nepoch + 1):
        ...
        acc = test_with_snr(model, dataloaders[-1])
        ...
        if acc > max_acc:
            max_acc = acc
            #stop = 0
            torch.save(model.state_dict(), 'wo_pretrain_snr_h_mmd.pth')
    ...
    print('testing.............')
    model = DNET_lmmd()
    model.load_state_dict(torch.load('wo_pretrain_snr_h_mmd.pth'))
    model.cuda()
    model.eval()
    correct = 0
    fals = 0
    ...
    for i, data in enumerate(dataloaders[-1]):
        testdata, testlabel, SNR = data
        testdata = torch.reshape(testdata, [-1, 2, 128])
        testdata = testdata.cuda()
        testlabel = testlabel.cuda()
